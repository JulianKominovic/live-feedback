...
const StyledResults = styled.div<{ active: boolean }>`
  background: ${({ active }) =>
...
function metricScoreToColorScheme(score?: IVitalsScore | "needs-improvement") {
  switch (score) {
    case "good":
      return "success";
...
export default function Cmdk({ shadowRoot }: { shadowRoot: ShadowRoot }) {
  ...
  const fixedActions: (Omit<Action, "name"> & {
  ...
  return (
    <KBarPortal container={shadowRoot as unknown as HTMLElement}>
      <KBarPositioner
        style={{
